...
def mysurvey():
    ...
    surveys=db(db.survey.id==request.args[0])(db.survey.own==session.person_id).select()
    if len(surveys)<1:
...
def thissurvey():
    ...
    if not len(rows):
        ...
        redirect(URL(r=request,f='index'))
    rows2=db(db.sa.own==person_id)(db.sa.survey==survey_id).select()
    if not len(rows2):
       id=db.sa.insert(survey=survey_id,own=person_id,completed=False,
