...
db.membership.persons_group.requires=IS_IN_DB(db,'persons_group.id','%(id)s')

db.define_table('acc',
   Field('persons_group',db.persons_group),
   Field('table_name', default='somename'),
   Field('record_id','integer', default='0'),
   Field('access_type', default ='read'))

db.acc.persons_group.requires=IS_IN_DB(db,'persons_group.id')
db.acc.access_type.requires=IS_IN_SET(['read','edit'])
...
def has_access(person_id,table_name,record_id,access_types):
    ...
    rows=db(db.acc.table_name==table_name)\
           ...
           (db.acc.access_type.belongs(access_types))\
