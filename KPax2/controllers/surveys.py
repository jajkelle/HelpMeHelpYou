...
def create_survey():
    ...
    form.vars.own=auth.user_id
    if form.accepts(request,session):
        db.acc.insert(table_name='survey',record_id=form.vars.id,\
                         persons_group=g_tuple[1],access_type="take")
        db.sa.insert(survey=form.vars.id,own=auth.user_id)
...
def edit_questions():
    ...
    rows=db(db.question.survey==survey.id)\
           ...
           (db.survey.own==auth.user_id).select(orderby=db.question.number)
    ...
    if question_id>0:
        qrows=db(db.question.id==question_id)\
                ...
                (db.survey.own==auth.user_id).select()
        if len(qrows):
            form=SQLFORM(db.question,qrows[0].question,showid=False,
                 fields=question_fields,deletable=db.survey.own==auth.user_id)
            if form.accepts(request,session):
...
def report():
    ...
    if len(request.args)<2:
       sas=db(db.sa.survey==survey.id)\
             (db.sa.own==db.auth_user.id).select(orderby=db.auth_user.name)
       rows=db(db.question.survey==survey.id)\
              (db.answer.question==db.question.id)\
              (db.answer.sa==db.sa.id)\
              (db.auth_user.id==db.sa.own).select(orderby=db.question.id|db.auth_user.name)
    else:
       sas=db(db.sa.own==request.args[1])\
             (db.sa.survey==survey.id)\
             (db.sa.own==db.auth_user.id).select(orderby=db.auth_user.name)
       rows=db(db.question.survey==survey.id)\
              (db.answer.question==db.question.id)\
              (db.answer.sa==db.sa.id)\
              (db.auth_user.id==db.sa.own)\
...
def delete_sa():
    ...
    if int(request.args[1])!=auth.user_id:
        db(db.sa.own==request.args[1])(db.sa.survey==survey.id).delete()
...
def upload_survey():
    ...
    try:
        i=db.survey.insert(title=data.title,\
                           ...
                           own=auth.user_id)
        db.acc.insert(table_name='survey',record_id=i,\
                         persons_group=g_tuple[1],access_type="take")
        db.sa.insert(survey=i,own=auth.user_id)
    except:
