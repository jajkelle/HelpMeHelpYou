...
@auth.requires_login()
def index():
    chats=accessible('chat_line',('read','read/chat'))(db.chat_line.own==db.auth_user.id).select(orderby=db.chat_line.name,groupby=db.chat_line.id)
...
def open_chat():
    ...
    access=has_acc(auth.user_id,'chat_line',chat_id,('read','read/chat'))
    if not access:
...
def clear():
    chat_id=request.args[0]
    if len(db(db.chat_line.id==chat_id)(db.chat_line.own==auth.user_id).select()):
