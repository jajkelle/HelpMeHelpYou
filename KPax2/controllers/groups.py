...
def index():
    ...
    mygroups=db(db.persons_group.id.belongs(g_tuple))\
               (db.persons_group.own==db.auth_user.id).select(orderby=db.persons_group.name.upper())
...
@auth.requires_login()
def approve():
    if len(request.args) and len(db(db.persons_group.id==request.args[0])(db.persons_group.own==auth.user_id).select()):
...
def edit_group():
    ...
    rows=db(db.persons_group.id==request.args[0])\
           (db.persons_group.own==auth.user_id).select()
