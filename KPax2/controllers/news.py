...
def index():
    announcements=accessible('announcement')\
         ...
         (db.auth_user.id==db.announcement.own)\
...
def post_announcement():
    ...
    if form.accepts(request,session):
        id=db.acc.insert(persons_group=g_tuple[1],table_name='announcement',\
...
def edit_announcement():
    ...
    rows=db(db.announcement.id==request.args[0])\
           (db.announcement.own==auth.user_id).select()
...
def rss():
    ...
    entries=db(db.announcement.id==db.acc.record_id)\
            ...
            (db.acc.persons_group.belongs(requested_groups))\
