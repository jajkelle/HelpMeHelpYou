...
response.menu += [
    (T('Resources'), False,'#', [
      (T('My Resources'), False,URL('list_resources',args = user_id) if session.auth else URL('default','index')),
      ...
      (T('Comment'), False, URL('show', args = user_id) if session.auth else URL('default','index_2')),
      (T('Upload'), False, URL('uploadimage') if session.auth else URL('default','uploadimage')) ]
    )
]
