...
def index():
    ...
    if form.accepts(request,formname='join_group'):
        ...
        elif has_acc(auth.user_id,'persons_group',persons_group,'see/join') or \
                groups[0].open==True:
    ...
    mygroups=db(db.persons_group.id.belongs(g_tuple))\
               (db.persons_group.owned_by==db.auth_user.id).select(orderby=db.persons_group.name.upper())
...
@auth.requires_login()
def approve():
    if len(request.args) and len(db(db.persons_group.id==request.args[0])(db.persons_group.owned_by==auth.user_id).select()):
...
def edit_group():
    ...
    rows=db(db.persons_group.id==request.args[0])\
           (db.persons_group.owned_by==auth.user_id).select()
