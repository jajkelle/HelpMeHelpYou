...
@auth.requires_login()
def index():
    chats=accessible('chat_line',('read','read/chat'))(db.chat_line.owned_by==db.auth_user.id).select(orderby=db.chat_line.username,groupby=db.chat_line.id)
    return dict(chats=find_groups(chats))

@auth.requires_login()
def create_chat():
    form=SQLFORM(db.chat_line,fields=db.chat_line.public_fields)
    form.vars.owned_by=auth.user_id
    if form.accepts(request,session):
        id=db.acc.insert(persons_group=g_tuple[1],table_name='chat_line',\
                            record_id=form.vars.id,acc_type='read/chat')
...
def edit_chat():
    ...
    rows=db(db.chat_line.id==request.args[0])\
           (db.chat_line.owned_by==auth.user_id).select()
...
def open_chat():
    ...
    acc=has_acc(auth.user_id,'chat_line',chat_id,('read','read/chat'))
    if not acc:
        session.flash='acc denied'
        redirect(URL('index'))
    chat_line=db(db.chat_line.id==chat_id).select()[0]
    readonly=acc.acc_type=='read'
    return dict(readonly=readonly,chat_line=chat_line)

@auth.requires_login()
def clear():
    chat_id=request.args[0]
    if len(db(db.chat_line.id==chat_id)(db.chat_line.owned_by==auth.user_id).select()):
...
def post():
    chat_id=request.args(0)
    acc=has_acc(auth.user_id,'chat_line',chat_id,('read','read/chat'))
    if not acc: raise HTTP(400)
    if request.vars.message and acc.acc_type=='read/chat':
    ...
    posts=[[m.message.id,str(m.message.posted_on),\
            cgi.escape(m.auth_user.username),m.auth_user.email,
...
def rss():
    ...
    entries=db(db.announcement.id==db.acc.record_id)\
            (db.acc.table_name=='announcement')\
            (db.acc.auth_users_group.belongs(requested_groups))\
            (db.announcement.to_rss==True)\
            (db.auth_user.id==db.announcement.owned_by)\
