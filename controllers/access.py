...
def change():
    ...
    rows=db(db.acc.table_name==table_name)\
           ...
           (db.acc.persons_group==db.persons_group.id)\
           .select(orderby=db.acc.id)
    group_keys=dict([(r.persons_group.id,r.acc.access_type) for r in rows])
    keys=dict([(r.acc.id,r.acc.access_type) for r in rows])
    accesses=[]
